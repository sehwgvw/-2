...
def kb_main(uid):
    btns = [
        [InlineKeyboardButton(text="🌸 Наказать нарушителя", callback_data="atk")],
...
async def start(m: Message, state: FSMContext):
    ...
    # ИСПРАВЛЕНИЕ: Проверяем доступ через подписку ИЛИ наличие в users.json
    users = load_json(FILES['users'], [])
    if check_access(uid) or str(uid) in map(str, users):
        s = cfg.get_settings()
...
async def auth_check(m: Message, state: FSMContext):
    ...
    if m.text.strip() in keys:
        ...
        await m.answer("✅ Ключ принят! Доступ активирован.")
        await start(m, state)
    else:
...
async def input_ask(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.answer("⌨️ Введи новое число:")
    await c.answer()
...
async def atk_3_y(c: CallbackQuery, state: FSMContext):
    await c.message.answer("Отправьте фото:")
...
async def go_home(c: CallbackQuery):
    ...
    # ИСПРАВЛЕНИЕ: Та же проверка доступа
    users = load_json(FILES['users'], [])
    if check_access(uid) or str(uid) in map(str, users):
